...
struct node {
    ...
    node(long long _sum, long long _mnPSum) {
        sum = _sum;
        mnPSum = _mnPSum;
    }
};
...
node op(node a, node b) {
    node c(0,0);
    c.sum = a.sum + b.sum;
    c.mnPSum = min(a.mnPSum,a.sum+b.mnPSum);
    return c;
}
...
void solve() {
    ...
    vector<node> arr(n+1,node(0,0));
    segtree<node,op,e> st(n+1, arr);
    ...
    for(int it = 0; it < q; it++) {
        ...
        long long sum = st.query(a,b+1).sum;
        long long mnPSum = st.query(a,b+1).mnPSum;
        
        if(sum == 0 and mnPSum == 0) {
            cout << "Yes\n";
            st.update(a,node(1,1));
            st.update(b,node(-1,-1));
        }
        else {
            cout << "No\n";
        }
