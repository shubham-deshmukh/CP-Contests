...
struct SegmentTree {
    ...
    SegmentTree(int n, vector<T> &arr) {
        ...
        size = 4*n;
        tree.assign(size,{}); // update the value (0) in case custom data structure is used like struct etc.
...
void solve() {
    int n, q; // buildings, queries
    cin >> n >> q;

    vector<long long> prices(n);
    for(auto &pi: prices) cin >> pi;

    vector<node> B(n);
    for(int i = 0; i < n; i++) {
        B[i] = {prices[i]-i,prices[i]+i};
    }

    SegmentTree<node> st(n, B);
    ...
    while(q--) {
        ...
        else if(type == 2) { // range queries
            long long k;
            cin >> k, k--;
            long long left = max(st.query(0,k+1).left + k, 0LL);
            long long right = max(st.query(k,n).right - k, 0LL);
            cout << min({prices[k], left, right});
            if(q) cout << endl;
        }
    }
}
