...
struct node {
    long long mxSum, mxPref, mxSuff, sum;
};
...
struct SegmentTree {
    ...
    T op(T a, T b) {
        return {
          max({a.mxSum, b.mxSum, a.mxSuff + b.mxPref}),
          max(a.mxPref, a.sum + b.mxPref),
          max(a.mxSuff + b.sum, b.mxSuff),
          a.sum + b.sum
        };
    }
...
void solve() {
    ...
    while(q--) {
        long long pos, val;
        cin >> pos >> val, pos--;
        st.update(pos, {val,val,val,val});
        cout << max(st.query(0,n).mxSum, 0LL);
        if(q) cout << endl;
    }
}
signed main() {
