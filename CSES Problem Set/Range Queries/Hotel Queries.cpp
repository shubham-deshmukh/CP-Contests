...
struct SegmentTree {
    ...
    T get(int v, int lb, int rb, int pos) {
        ...
        T res;
        if(pos <= mid) {
            res = get(2*v+1, lb, mid, pos);
        }
        else 
            res = get(2*v+2, mid+1, rb, pos);
    ...
    int search(int v, int lb, int rb, T x) {
        if(lb == rb) {
            if(x <= tree[v])
                return lb;
            return -1;
        ...
        int pos = -1;
        if(tree[2*v+1] >= x) {
            return search(2*v+1,lb, mid, x);
        }

        return search(2*v+2,mid+1, rb, x);
    }

    int search(T x) {
        return search(0,0,n-1,x);
    }
...
void solve() {
    int n, m; // hotels, groups
    cin >> n >> m;

    vector<int> hotels(n);
    for(auto &h: hotels) cin >> h;

    SegmentTree<int> st(n, hotels);

    while(m--) {
        int room;
        cin >> room;

        int pos = st.search(room);
        if(pos == -1) cout << 0;
        else {
            cout << pos + 1;
            // int have = st.get(pos);
            // st.update(pos,have - room);
            hotels[pos] -= room;
            st.update(pos,hotels[pos]);
        }
        if(m) cout << " ";
    }
}
