...
struct node {
    long long cnt = 0;
    vector<long long> freq = vector<long long> (41,0);
};
...
struct SegmentTree {
    ...
    SegmentTree(int n, vector<T> &arr) {
        ...
        size = 4*n;
        tree.assign(size,e()); // update the value (0) in case custom data structure is used like struct etc.
