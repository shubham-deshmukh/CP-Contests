...
struct SegmentTree {
    ...
    T op(T a, T b) {
        T c;
        c.inv_cnt = a.inv_cnt + b.inv_cnt;
        long long sum = 0;
        for(int i = 40; i >= 1; i--) {
            long long x = b.freq[i];
            c.inv_cnt += sum * x;
            sum += a.freq[i];
        }
        for(int i = 1; i < 41; i++)
    ...
    SegmentTree(int n, vector<T> &arr) {
        ...
        size = 4*n;
        tree.resize(size); // update the value (0) in case custom data structure is used like struct etc.
