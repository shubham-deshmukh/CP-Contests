...
struct SegmentTree {
    ...
    T op(T a, T b) {
        ...
        t.seg = max({a.seg, b.seg, a.suff + b.pref});
    ...
    // l is included and r is excluded i.e. [l,r) 
    T query(int l, int r) { 
        return query(0,0,n-1,l,r-1); // [l,r]
    }
...
void solve() {
    ...
    // print maximal sum on whole segment
    cout << max(st.query(0,n).seg,0LL)<< endl;
    while(q--) {
        int i; long long val;
        cin >> i >> val;

        // update ith position with val
        st.update(i,node(val));
        // print the maximal sum on whole segment
        cout << max(st.query(0,n).seg,0LL);
        if(q) cout << endl;
    }
}
signed main() {
