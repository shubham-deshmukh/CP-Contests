...
struct SegmentTree {
    ...
    void build(int v, int lb, int rb) {
        if(lb == rb) {
            ...
            sumTree[v] = fun(sumTree[lv], sumTree[rv]);
            tree[v] = op(tree[lv],tree[rv]);
        }
    }

    void update(int v, int lb, int rb, int l, int r, T new_value) {
        if(r < lb || rb < l) 
            return;

        if(lb >= l && rb <= r) {
            tree[v] = op(tree[v], new_value);
            sumTree[v] = op(sumTree[v], new_value);
            return;
        }
        ...
        update(lv, lb, mid,l,r, new_value);
        update(rv,mid+1,rb,l,r, new_value);

        sumTree[v] = op(fun(sumTree[lv], sumTree[rv]),tree[v]);
    }
    ...
    T query(int v, int lb, int rb, int l, int r) {
        ...
        return op(fun(s1,s2),tree[v]);
    }

    // [l,r)
    void update(int l, int r, T new_value) {
        update(0,0,n-1,l, r-1,new_value);
    }
