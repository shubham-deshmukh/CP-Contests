...
struct SegmentTree {
    ...
    SegmentTree(int n, vector<T> &arr) {
        ...
        size = 4*n;
        tree.assign(size,0); // modify the value (0) in case custom data structure is used like struct etc.
        queryTree.assign(size,0);
    ...
    void modify(int v, int lb, int rb, int l, int r, T new_value) {
        ...
        if(lb >= l && rb <= r) {
            ...
            queryTree[v] = modify_op(queryTree[v], new_value);
            return;
        }
        ...
        queryTree[v] = query_op(queryTree[lv], queryTree[rv]);
        queryTree[v] = modify_op(queryTree[v],tree[v]);
    }
