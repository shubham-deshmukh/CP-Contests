...
void solve() {
    ...
    long long beauty = 0;
    long long maxOfMin = 0, minOfMax = LLONG_MAX;
    for(int i = 0; i < N; i++) {
        beauty += abs(A[i]-B[i]);
        maxOfMin = max(maxOfMin, min(A[i],B[i]));
        minOfMax = min(minOfMax, max(A[i],B[i]));
    }

    int extra = maxOfMin - minOfMax;
    beauty += (extra > 0? 2LL*extra: 0);
    cout << beauty;
}
int main() {
    // your code goes here
